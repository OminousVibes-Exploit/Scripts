...
return setclipboard("["..tostring(m).."] = "..tostring(n)..",")
end
end
do
local m=k..i
...
local s=b.LocalPlayer
local t=f.CurrentCamera

local u=c:WaitForChild"GameData"
...
do
local w=c:WaitForChild"EntityInfo"
local x={
A90=w:WaitForChild"A90",
}
...
L, M)
if L and Toggles["alerts.enabled"].Value then
local N={
...
n:Notify(N.message,N.duration)
if N.audio and Toggles["alerts.audio"].Value then
w.TimePosition=0.25
w:Play()
end
end end
...
do
local M=r
x=setmetatable({},{__index=M})
x.__index=x

function x.__tostring(N)
return"CharacterComponent - "..N.instance:GetFullName()
end
x.attached={}
...
O=O:constructor(N,...)or O
O:run()
return O
end


function x.constructor(N,O)
M.constructor(N,O)
N.client=A.attached[b:GetPlayerFromCharacter(O)]
N.root=O:WaitForChild("HumanoidRootPart",5)
N.humanoid=O:WaitForChild("Humanoid",5)

assert(N.client~=nil,"["..N:__tostring().."] is not a client's character")
assert(N.root~=nil,"["..N:__tostring().."] is missing a HumanoidRootPart")
assert(N.humanoid~=nil,"["..N:__tostring().."] is missing a Humanoid")

N._maid:add(O.AncestryChanged:Connect(function(P,Q)
if Q==nil then
N:destroy()
end
end))
...
function x.onStart(N)
x.attached[N.instance]=N
M.onStart(N)
end


function x.destroy(N)
x.attached[N.instance]=nil
M.destroy(N)
end
end
...
local M=Vector3.new(1,0,1)

local N=x
y=setmetatable({},{__index=N})
y.__index=y

function y.__tostring(O)
return"AvatarComponent - "..O.instance:GetFullName()
end
y.attached=nil


function y.new(O,...)
local P=setmetatable({},y)
...
function y.constructor(O,P)
N.constructor(O,P)
O.light=Instance.new"PointLight"
end


function y.onStart(O)
y.attached=O
N.onStart(O)
...
function y.onPhysics(O,P,Q)
local R=Options["character.sprint.speed"].Value
if
R>0
and Toggles["character.sprint.enabled"].Value
and Options["character.sprint.keybind"]:GetState()
then
local S=O.humanoid.MoveDirection*M
if S.Magnitude>0 then
O.instance:TranslateBy(S*R*Q)
end
end
end


function y.destroy(O)
y.attached=nil
N.destroy(O)
end
end
...
local M=Vector3.new(5,7,0)
local N=Vector3.new(0,-0.25,0)
local O,P,Q,R=
CFrame.new(N.X-(M.X/2),N.Y+(M.Y/2),N.Z),
CFrame.new(N.X+(M.X/2),N.Y+(M.Y/2),N.Z),
CFrame.new(N.X-(M.X/2),N.Y-(M.Y/2),N.Z),
CFrame.new(N.X+(M.X/2),N.Y-(M.Y/2),N.Z)
local S=t.WorldToViewportPoint
local T=Vector2.new
local U=math.min

local V=x
z=setmetatable({},{__index=V})
z.__index=z

function z.__tostring(W)
return"FriendComponent - "..W.instance:GetFullName()
end
z.attached={}


function z.new(W,...)
local X=setmetatable({},z)
...
function z.constructor(W,X)
V.constructor(W,X)
W.quad=Drawing.new"Quad"
W._maid:add(function()
W.quad:Remove()
end)
end


function z.onStart(W)
z.attached[W.instance]=W
V.onStart(W)
...
function z.onRender(W,X)
local Y=W.quad
if Toggles["esp.player.enabled"].Value then
local Z=W.root.CFrame
local _=S(t,(Z*O).Position)
local aa=S(t,(Z*P).Position)
local ab=S(t,(Z*R).Position)
local ac=S(t,(Z*Q).Position)
if U(_.Z,aa.Z,ab.Z,ac.Z)>0 then
Y.PointA=T(aa.X,aa.Y)
Y.PointB=T(_.X,_.Y)
Y.PointC=T(ac.X,ac.Y)
Y.PointD=T(ab.X,ab.Y)
Y.Color=Options["esp.player.color"].Value
Y.Visible=true
else
Y.Visible=false
end
...
function z.destroy(aa)
z.attached[aa.instance]=nil
V.destroy(aa)
end
end
...
do
local aa=r
A=setmetatable({},{__index=aa})
A.__index=A

function A.__tostring(ab)
...
function A.constructor(ab,ac)
aa.constructor(ab,ac)
end


function A.onStart(ab)
A.attached[ab.instance]=ab
aa.onStart(ab)
...
task.defer(function()
for ac,M in ipairs(ab.instance.Backpack:GetChildren())do
if M:IsA"Tool"then
ab:onTool(M)
end
end
...
function A.onCharacter(ab,ac)
if ab.instance==s then
ab.character=y.new(ac)
else
ab.character=z.new(ac)
end
end

function A.onTool(ab,ac)
if ac.Name=="LibraryHintPaper"then
local M=ac:WaitForChild("UI",1)
if M then
local N={}
for O =1,5 do
local P=M:FindFirstChild(tostring(O))
if P then
...
function A.destroy(ab)
A.attached[ab.instance]=nil
aa.destroy(ab)
end
...
for ac,M in ipairs(b:GetPlayers())do
task.defer(ab,M)
end
end
...
CFrame.new(ab.X-(aa.X/2),ab.Y-(aa.Y/2),ab.Z),
CFrame.new(ab.X+(aa.X/2),ab.Y-(aa.Y/2),ab.Z)
local P=t.WorldToViewportPoint
local Q=Vector2.new
local R=math.min
...
local X=P(t,(W*ac).Position)
local Y=P(t,(W*M).Position)
local Z=P(t,(W*O).Position)
local _=P(t,(W*N).Position)
...
function B.isEnabled(T)
return Toggles["esp.rush.enabled"].Value
end

function B.getColor(T)
...
local S=r
C=setmetatable({},{__index=S})
C.__index=C

function C.__tostring(T)
...
function C.onStart(T)
C.attached[T.instance]=T
S.onStart(T)
...
function C.isEnabled(T)
return Toggles["esp.rush.enabled"].Value
end

function C.getColor(T)
return Options["esp.rush.a120-color"].Value
end

function C.destroy(T)
C.attached[T.instance]=nil
S.destroy(T)
end
end
...
local ab=t.WorldToViewportPoint
local ac=Vector2.new
local M=math.min

local N=r
D=setmetatable({},{__index=N})
D.__index=D

function D.__tostring(O)
...
function D.onStart(O)
D.attached[O.instance]=O
N.onStart(O)
...
local T=ab(t,O.br)
local U=ab(t,O.bl)
if M(R.Z,S.Z,T.Z,U.Z)>0 then
Q.PointA=ac(S.X,S.Y)
Q.PointB=ac(R.X,R.Y)
Q.PointC=ac(U.X,U.Y)
Q.PointD=ac(T.X,T.Y)

local V=O.id-u.LatestRoom.Value
if V==0 then
...
local aa=Vector3.new(4.675,7.15,0)
local ab=Vector3.new(0,0,-1.25)
local ac,M,N,O=
CFrame.new(ab.X-(aa.X/2),ab.Y+(aa.Y/2),ab.Z),
CFrame.new(ab.X+(aa.X/2),ab.Y+(aa.Y/2),ab.Z),
CFrame.new(ab.X-(aa.X/2),ab.Y-(aa.Y/2),ab.Z),
...
V.Transparency=0.10
elseif u.LatestRoom.Value-T.id>1 then
V.Transparency=0.25
else
V.Transparency=0.60
end
...
local aa=t.WorldToViewportPoint
local ab=Vector2.new
local ac=ab(0,3)

local M=r
F=setmetatable({},{__index=M})
F.__index=F

function F.__tostring(N)
return"InteractableComponent - "..N.instance:GetFullName()
end
F.attached={}
...
P=P:constructor(N,O,...)or P
P:run()
return P
end


function F.constructor(N,O,P)
M.constructor(N,O)
N.name=P
N.label=Drawing.new"Text"
N.circle=Drawing.new"Circle"
N.outline=Drawing.new"Circle"
N._maid:add(function()
N.label:Remove()
N.circle:Remove()
N.outline:Remove()
end)
N._maid:add(O.AncestryChanged:Connect(function(Q,R)
if R==nil then
N:destroy()
end
end))
...
function F.onStart(N)
F.attached[N.instance]=N
M.onStart(N)

local O,P,Q=N.label,N.circle,N.outline
O.Text=N.name
O.Size=18
O.Center=true
O.Outline=true
...
function F.onRender(N,O)
local P,Q,R=N.label,N.circle,N.outline
if N:isVisualEnabled()then
local S=aa(t,N.instance.Position)
local T=S.Z>0
if T then
local U=ab(S.X,S.Y)
P.Position=U+ac
...
function F.isVisualEnabled(N)
return Toggles["esp.interactable.enabled"].Value
end

function F.getColor(N)
return Options["esp.interactable.color"].Value
end

function F.destroy(N)
F.attached[N.instance]=nil
M.destroy(N)
end
end
...
do
local aa=F
G=setmetatable({},{__index=aa})
G.__index=G

function G.__tostring(ab)
return"ObtainableComponent - "..ab.instance:GetFullName()
end
G.attached={}


function G.new(ab,ac,...)
local M=setmetatable({},G)
M=M:constructor(ab,ac,...)or M
M:run()
return M
end


function G.constructor(ab,ac,M)
aa.constructor(ab,ac,M)
end


function G.onStart(ab)
G.attached[ab.instance]=ab
aa.onStart(ab)
local ac,M,N=ab.label,ab.circle,ab.outline
M.NumSides=5
N.NumSides=5
end


function G.isVisualEnabled(ab)
return Toggles["esp.obtainable.enabled"].Value
end

function G.getColor(ab)
return Options["esp.obtainable.color"].Value
end

function G.destroy(ab)
G.attached[ab.instance]=nil
aa.destroy(ab)
end
end
...
local aa=r
H=setmetatable({},{__index=aa})
H.__index=H

function H.__tostring(ab)
...
function H.onStart(ab)
H.attached[ab.instance]=ab
aa.onStart(ab)
...
ab._maid:add(ab.instance.DescendantAdded:Connect(function(M)
ab:onDescendant(M)
end))
for M,N in ipairs(ab.instance:GetDescendants())do
ab:onDescendant(N)
if M%1000==0 then
task.wait()
end
...
function H.onDescendant(ab,ac)
local M=ac.Parent
local N=M.Name
local O=ac.Name
if O=="Main"then
if N=="LeverForGate"then
F.new(ac,"Lever")
elseif N=="Lighter"then
G.new(ac,"Lighter")
elseif N=="Vitamins"then
G.new(ac,"Vitamins")
elseif N=="Lockpick"then
G.new(ac,"Lockpicks")
elseif N=="Bandage"then
G.new(ac,"Bandages")
elseif N=="Battery"then
G.new(ac,"Batteries")
end
elseif O=="Holder"then
if N=="CrucifixWall"then
G.new(ac,"Crucifix")
end
elseif O=="Rooms_Locker"then
E.new(ac,ab.id)
...
function H.destroy(ab)
H.attached[ab.instance]=nil
aa.destroy(ab)
end
end
...
n:SetWatermark"Linoria Community (TerminalVibes)"
n:Notify"Loading UI..."

do
local aa=n:CreateWindow(h.." ("..i..")")
...
ac:AddToggle("gameplay.a90.disable-movement",{
...
do
local ab=aa:AddTab"Blatant"

do
local ac=ab:AddLeftGroupbox"Entity Hooks"

ac:AddToggle("blatant.a90.hook-damage",{
Text="A90 immunity",
Default=false,
Tooltip="You will not be damaged by A90.",
})
end
end
...
local N=ac:AddTab"Object ESP"
do
N:AddToggle("esp.door.enabled",{Text="Door ESP",Default=false,Tooltip="ESP for doors"})
...
local M=ab:AddRightGroupbox"Socials"
M:AddButton{
Text="Discord Server",
Func=function()
...
f.ChildAdded:Connect(ab)

local ac=function(ac)
H.new(ac)
end
f.CurrentRooms.ChildAdded:Connect(ac)
for M,N in ipairs(f.CurrentRooms:GetChildren())do
ac(N)
end



v.A90.OnClientEvent:Connect(function()
if Toggles["gameplay.a90.disable-movement"].Value then
task.wait(0.6)


local O=y.attached
O.root.AssemblyLinearVelocity=Vector3.zero
...
for R,S in pairs(Q.attached)do
S:onTick(O)
end
end
end
...
d.Heartbeat:Connect(O)
d.RenderStepped:Connect(P)
d.Stepped:Connect(Q)
...
local U=Toggles["blatant.a90.hook-damage"]


R=hookmetamethod(game,"__namecall",function(V,...)
if checkcaller()then
return R(V,...)
end
...
w.Name="Main"
w.SoundId="rbxassetid://6026984224"
w.Volume=6
w.Parent=e
...
setclipboard("["..tostring(R).."] = "..tostring(S)..",")
end
end

return n:Notify("["..h.."] Loaded! ("..i..")")
